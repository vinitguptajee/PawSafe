...
import { ReactComponent as PawSafeLogo } from "./assets/pawsafe-logo.svg";
...
const AGGRESSION_TYPES = [
 {
   ...
   icon: <MapPin className="w-6 h-6 text-amber-700 dark:text-amber-500" />, // Changed to brown
...
const Button = ({ children, primary = false, className = "", ...props }) => (
 <button 
   className={`px-6 py-3 rounded-full font-medium transition-all duration-300 transform hover:scale-105 active:scale-95 flex items-center justify-center gap-2 ${
     primary 
       ? "bg-gradient-to-r from-amber-800 to-orange-900 text-white shadow-lg shadow-amber-900/20 hover:shadow-amber-900/40 border border-transparent" 
...
const SectionHeading = ({ children, subtitle, darkBg = false }) => (
 <div className="mb-12 text-center relative z-10">
   ...
   {subtitle && <p className={`text-lg max-w-2xl mx-auto font-light ${darkBg ? 'text-amber-100' : 'text-stone-600 dark:text-stone-400'}`}>{subtitle}</p>}
 </div>
);
...
const FeatureCard = ({ icon, title, desc }) => (
 <div className="p-6 rounded-2xl bg-white border border-stone-200 shadow-sm hover:shadow-xl hover:shadow-stone-200/50 dark:bg-stone-800/80 dark:border-stone-700 dark:shadow-none backdrop-blur-sm hover:translate-y-[-4px] transition-all duration-300 group">
   <div className="mb-4 p-3 bg-stone-100 dark:bg-stone-700 rounded-xl w-fit group-hover:bg-amber-50 dark:group-hover:bg-amber-900/30 transition-colors">
     {React.cloneElement(icon, { className: "w-8 h-8 text-stone-600 dark:text-stone-300 group-hover:text-amber-700 dark:group-hover:text-amber-500 transition-colors" })}
   </div>
       ...
       <div className="flex items-center justify-between h-20">
         ...
         <div className="hidden md:flex items-center space-x-8">
           <div className="flex items-baseline space-x-8">
             {navLinks.map((link) => (
               <a key={link.name} href={link.href} className="text-stone-600 hover:text-amber-800 dark:text-stone-300 dark:hover:text-amber-500 transition-colors text-sm font-medium">
                 {link.name}
               </a>
             ))}
           </div>
         ...
         <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
           {navLinks.map((link) => (
             <a 
               key={link.name} 
               href={link.href} 
               className="block px-3 py-2 rounded-md text-base font-medium text-stone-600 dark:text-stone-300 hover:text-amber-800 dark:hover:text-amber-500 hover:bg-stone-100 dark:hover:bg-stone-800"
...
export default function App() {
 ...
 return (
   <div className={isDarkMode ? 'dark' : ''}>
     <div className="min-h-screen bg-stone-50 text-stone-900 dark:bg-stone-950 dark:text-stone-200 font-sans transition-colors duration-300 selection:bg-amber-500/30">
       <Navbar isDarkMode={isDarkMode} toggleTheme={toggleTheme} />

       {/* Hero Section */}
       <section className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 overflow-hidden">
         {/* Background decorations - Earthy blobs */}
         <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[1000px] h-[500px] bg-amber-800/10 dark:bg-amber-900/20 rounded-full blur-[120px] -z-10" />
         <div className="absolute top-40 right-0 w-[500px] h-[500px] bg-orange-500/5 dark:bg-orange-600/10 rounded-full blur-[100px] -z-10" />
         ...
         <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
           <div className="flex flex-col lg:flex-row items-center gap-16">
             <div className="flex-1 text-center lg:text-left z-10">
               <div className="inline-flex items-center space-x-2 bg-white dark:bg-stone-800/50 rounded-full px-4 py-1.5 border border-stone-200 dark:border-stone-700 mb-8 backdrop-blur-sm shadow-sm">
                 ...
                 <span className="text-sm font-medium text-stone-600 dark:text-amber-400">New: AI Dog Behavior Scanner</span>
               </div>
               <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight text-stone-800 dark:text-stone-100 mb-6 leading-tight font-serif">
                 Coexist with <br />
                 <span className="bg-clip-text text-transparent bg-gradient-to-r from-amber-800 to-orange-700 dark:from-amber-500 dark:to-orange-400">
                   Nature & Neighbors
                 </span>
               </h1>
           ...
           <div className="mt-20 relative mx-auto max-w-4xl transform hover:translate-y-[-5px] transition-transform duration-500">
             <div className="relative rounded-2xl bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-700 shadow-2xl overflow-hidden">
               <div className="absolute top-0 w-full h-1 bg-gradient-to-r from-amber-600 via-orange-700 to-amber-900"></div>
         ...
         <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
           ...
           <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
             {AGGRESSION_TYPES.map((item, index) => (
               <div key={index} className="bg-white dark:bg-stone-900 rounded-2xl p-6 border border-stone-200 dark:border-stone-800 hover:border-amber-700/50 dark:hover:border-amber-500/50 transition-all hover:-translate-y-1 shadow-lg dark:shadow-none">
                 ...
                 <div className="p-3 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-100 dark:border-amber-900/30">
                   <p className="text-amber-800 dark:text-amber-500 text-xs font-bold uppercase tracking-wider mb-1">How to handle</p>
           ...
           <div className="grid lg:grid-cols-2 gap-16 items-center">
             <div className="order-2 lg:order-1">
               <h2 className="text-4xl font-bold text-stone-800 dark:text-stone-100 mb-6 font-serif">
                 Technology for <br/>
                 <span className="text-amber-800 dark:text-amber-500">Safer Communities</span>
               </h2>
               ...
               <div className="space-y-8">
                 ...
                 <div className="flex gap-4 group">
                   <div className="flex-shrink-0 w-12 h-12 rounded-full bg-amber-100 dark:bg-amber-500/10 flex items-center justify-center group-hover:bg-amber-200 dark:group-hover:bg-amber-500/20 transition-colors">
                     <MapPin className="w-6 h-6 text-amber-700 dark:text-amber-500" />
                   </div>
                   <div>
                 ...
                 <div className="flex gap-4 group">
                   <div className="flex-shrink-0 w-12 h-12 rounded-full bg-stone-100 dark:bg-stone-500/10 flex items-center justify-center group-hover:bg-stone-200 dark:group-hover:bg-stone-500/20 transition-colors">
               ...
               <div className="relative rounded-3xl overflow-hidden shadow-2xl border-4 border-stone-100 dark:border-stone-800">
                 ...
                 <div className="absolute bottom-6 left-6 right-6 bg-white/95 dark:bg-stone-900/95 backdrop-blur-md rounded-xl p-6 shadow-lg border border-stone-200 dark:border-stone-700">
                    <div className="flex items-center gap-2 mb-3 border-b border-stone-200 dark:border-stone-700 pb-2">
                       <BookOpen className="w-5 h-5 text-amber-800 dark:text-amber-500"/>
                    ...
                    <ul className="space-y-2">
                       {LEGAL_FACTS.slice(0, 3).map((fact, i) => (
                         <li key={i} className="flex gap-2 text-xs text-stone-700 dark:text-stone-300">
                           <CheckCircle className="w-4 h-4 text-amber-700 dark:text-amber-500 flex-shrink-0" />
                           {fact}
                         </li>
                       ))}
                    </ul>
       ...
       <section id="community" className="py-20 relative bg-stone-900">
         ...
         <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              {GUIDELINES.map((section, idx) => (
                <div key={idx} className="bg-stone-900/40 rounded-3xl p-8 border border-white/10 backdrop-blur-md hover:bg-stone-900/60 transition-colors">
                  <h3 className="text-2xl font-bold text-stone-100 mb-6 border-l-4 border-amber-600 pl-4">{section.category}</h3>
            ...
            <div className="mt-16 bg-gradient-to-r from-amber-900 to-orange-950 rounded-3xl p-8 md:p-12 text-center relative overflow-hidden shadow-2xl border border-amber-800/50">
               <div className="absolute top-0 right-0 p-12 opacity-10 transform rotate-12">
                  <Users className="w-64 h-64 text-amber-100" />
               </div>
               <h3 className="text-3xl font-bold text-white mb-4 relative z-10 font-serif">Start a PawSafe Chapter</h3>
               <p className="text-amber-100 max-w-2xl mx-auto mb-8 relative z-10 font-light">
                 Empower your Gated Community or University with the tools to manage dog-human conflict scientifically and humanely.
               </p>
               <div className="relative z-10">
                 <Button className="bg-white text-amber-900 hover:bg-amber-50 border-none shadow-xl font-bold">
         ...
         <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
           <div className="grid grid-cols-1 md:grid-cols-4 gap-12">
             ...
             <div>
               ...
               <ul className="space-y-2 text-stone-600 dark:text-stone-500 text-sm">
                 <li><a href="#issue" className="hover:text-amber-700 dark:hover:text-amber-500">About Us</a></li>
                 <li><a href="#community" className="hover:text-amber-700 dark:hover:text-amber-500">Careers</a></li>
                 <li><a href="#issue" className="hover:text-amber-700 dark:hover:text-amber-500">Privacy Policy</a></li>
                 <li><a href="#issue" className="hover:text-amber-700 dark:hover:text-amber-500">Contact</a></li>
               </ul>
             </div>
           </div>
